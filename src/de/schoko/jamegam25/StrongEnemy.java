...
public class StrongEnemy extends GameObject {
    ...
    public StrongEnemy(Game game, Player player, double x, double y) {
        super(x, y);
		this.game = game;
		this.player = player;
		this.health = 200;
		this.drawing = new ImageFrame(x, y, game.getContext().getImagePool().getImage("enemyStrong"), 16);
		images = new Image[] {
			game.getContext().getImagePool().getImage("enemyStrongLeft"), 
			game.getContext().getImagePool().getImage("enemyStrongRight"),
			game.getContext().getImagePool().getImage("enemyStrongLeftDamage"), 
			game.getContext().getImagePool().getImage("enemyStrongRightDamage")};
    }
    ...
    public void render(Graph g, double deltaTimeMS) {
		...
		// Damage to player
		if (player.x > this.x - 1 && player.x < this.x + 1 && player.y > this.y - 1 && player.y < this.y + 1) {
			this.vx -= Math.cos(direction) * deltaTimeMS / 1000 * speed * 1.25;
        	this.vy -= Math.sin(direction) * deltaTimeMS / 1000 * speed * 1.25;
